...
// Konfigurasi
// Tidak ada SoftwareSerial, kita akan menggunakan 'Serial' bawaan
...
// Fungsi parser UBX (menggunakan Hardware 'Serial')
bool parseUbx() {
  ...
  while (Serial.available() > 0) {
    ...
    switch (currentState) {
      ...
      case STATE_CHECKSUM:
        ...
        if (b == ck_a && Serial.read() == ck_b) { return true; }
        return false;
    }
  }
  return false;
...
void setup() {
  // 'Serial' sekarang digunakan khusus untuk GPS
  Serial.begin(115200);
...
void loop() {
  if (parseUbx()) {
    // Proses data kecepatan jika lokasi sudah valid
    if (isLocationValid()) {
    ...
    // Gambar ulang LCD hanya jika ada data baru
    u8g2.firstPage();
    do {
      drawStatusBox();
